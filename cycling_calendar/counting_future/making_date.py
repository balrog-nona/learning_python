...
timeMin1 must be the first day of last month because I will gather
    the events from the whole last month.
timeMax1 must be the last day of last month.
"""
last_month = date.today() - relativedelta(months=1)  # date object
time_min1 = date.today().replace(day=1, month=last_month.month,
                                 year=last_month.year).isoformat() + "T01:00:00Z"

length_last_month = monthrange(last_month.year, last_month.month)[1]
time_max1 = date.today().replace(day=length_last_month, month=last_month.month,
                                 year=last_month.year).isoformat()
time_max1 += "T23:50:00Z"

"""
Creating dates to use in the SECOND call to the calendar API
I'm looking for the last event called "rotoped soucet" which was created last day
of penutimate month as a whole day event.
timeMin2 must be the last day of penultimate month.
timeMax2 must be the first day of the last month because that's how google calendar works -
the whole day events end the next day.
"""
...
time_max2 = date.today().replace(day=1, month=last_month.month,
                                 year=last_month.year).isoformat() + "T23:00:00Z"
...
"""
start_date = date.today().replace(day=length_last_month, month=last_month.month,
                                  year=last_month.year).isoformat()
