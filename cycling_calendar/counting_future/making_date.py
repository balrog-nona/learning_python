...
time_min1 = datetime.date.today().replace(month=last_month_number).isoformat() + "T01:00:00Z"
...
time_max2 = datetime.date.today().replace(month=last_month_number).isoformat() + "T23:00:00Z"

"""
Creating dates for the new event
This event will be the whole day event which means:
start must be yesterday - the last day of last month
end must be today - the first day of current month
"""
