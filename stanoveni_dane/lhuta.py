...
class Lhuta:
    ...
    def __init__(self, zacatek):
        ...
        self.konec = self.zacatek.replace(year=self.zacatek.year + 3)  # normalne dle odst. 1
        self.maximalni_delka = self.zacatek.replace(year=self.zacatek.year + 10)  # dle odst. 5
    ...
    def odst3(self, datum):
        ...
        if self.kontrola_pred_zacatkem(datum=ukon) and self.kontrola_po_konci(datum=ukon):
            ...
            druhy_den = druhy_den.strftime('%d.%m.%Y')  # prevod datetime na string
            druhy_den = self.na_americke_datum(druhy_den)  # prevod stringu na date object
