"""
Testy testuji vsechny metody ze trid definovanych v lhuta_komplet.py -
kvuli tomu se pristupuje do protected metod
Testuji se i vsechny vijimky(kontroly) + hlavne metodi konec_lhuty, ktere
provadeji samotny vypocet delky lhuty
"""
...
def test_lhuta148():
    """
    test na tridu Lhuta148 a jeji metody a definovane vyjimky
    """
    lhuta = Lhuta148()
    ...
    assert lhuta._kontrola_pred_zacatkem(datum_ukonu=datetime.date(2019, 7, 23),
                                         datum_zacatku=datetime.date(2019, 7, 23))
    with pytest.raises(Exception,
                       match='Ukon ze dne 22.07.2019 nemuze nastat pred '
                             'zapocetim behu lhuty dne 23.07.2019.'):
        lhuta._kontrola_pred_zacatkem(datum_ukonu=datetime.date(2019, 7, 22),
                                      datum_zacatku=datetime.date(2019, 7, 23))

    assert lhuta._kontrola_po_konci(datum_ukonu=datetime.date(2020, 6, 13),
                                    datum_konce=datetime.date(2020, 6, 13))

    with pytest.raises(Exception,
                       match='Ukon ze dne 14.06.2020 nemuze nastat po '
                             'konci behu lhuty dne 13.06.2020.'):
        lhuta._kontrola_po_konci(datum_ukonu=datetime.date(2020, 6, 14),
                                 datum_konce=datetime.date(2020, 6, 13))

    assert lhuta._kontrola_odst5(konec=datetime.date(2019, 4, 1),
                                 maximalni_delka=datetime.date(2019, 4, 1))
    assert lhuta._kontrola_odst5(konec=datetime.date(2019, 3, 31),
                                 maximalni_delka=datetime.date(2019, 4, 1))
    assert not lhuta._kontrola_odst5(konec=datetime.date(2019, 4, 2),
                                     maximalni_delka=datetime.date(2019, 4, 1))
...
def test_odst2():
    ...
    odst2.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst1.konec,
...
def test_odst3():
    ...
    odst3.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst1.konec,
                      maximalni_delka=odst1.maximalni)
    assert odst3._vrat_konec(datum=odst3.konec) == '22.09.2025'

    extension = Odst3(datum='11.3.2025')
    extension.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst3.konec,
                          maximalni_delka=odst1.maximalni)
    assert extension._vrat_konec(datum=extension.konec) == '13.03.2028'

    extension2 = Odst3(datum='13.03.2028')
    extension2.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=extension.konec,
                           maximalni_delka=odst1.maximalni)
...
def test_odst4():
    ...
    odst4.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst1.konec,
                      maximalni_delka=odst1.maximalni)
    ...
    extension.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst4.konec,
                          maximalni_delka=odst1.maximalni)
...
def test_kompletni1():
    ...
    odst2.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst3.konec,
                      maximalni_delka=odst1.maximalni)
    ...
    odst4.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst2.konec,
                      maximalni_delka=odst1.maximalni)
...
def test_kompetni2():
    ...
    odst2.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=konec,
                      maximalni_delka=odst1.maximalni)
    assert odst2.konec == datetime.date(2025, 10, 9)
    ...
    odst4.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=konec,
                      maximalni_delka=odst1.maximalni)
    assert odst4.konec == datetime.date(2026, 7, 30)

    odst2 = Odst2(datum='22.5.2025')  # R o odvolani
    odst2.konec_lhuty(datum_zacatku=odst1.ukon, datum_konce=odst4.konec,
                      maximalni_delka=odst1.maximalni)
